...
export abstract class spell{
    ...
    apply(player: Player){
        if(player.game.physics.arcade.distanceBetween(player, player.target) < player.scope){
            if(!player.using_spell && player.energy >= this.energyCost && player.game.time.now - this._last_use > this._coolDownTime){
                ...
                timer.add(this.castTime+1, this._end_spell.bind(this, player));
    ...
    private _end_spell(player: Player){
...
export class spell_heal extends spell{
    constructor(){
        super('Heal', Phaser.Timer.SECOND, 5*Phaser.Timer.SECOND, 10);
    }
    _effects(player: Player){
        player.target.heal(100);
    }
}
export class spell_nightmare extends spell{
    private _timer: Phaser.Timer;
    private _repetition: number = 5;
    constructor(){
        super('Nightmare', Phaser.Timer.SECOND, 2*Phaser.Timer.SECOND, 15, 50);

    }
    _effects(player: Player){
        if(!this._timer){
            this._timer = player.game.time.create();
            this._timer.loop(Phaser.Timer.SECOND, this._effect.bind(this, player.target));
        }
        this._timer.start();
    }
    private _effect(target: Phaser.Sprite){
        if(target.game.time.now - this._last_use > this._repetition * Phaser.Timer.SECOND) this._timer.stop();
        target.damage(15);
    }
}
