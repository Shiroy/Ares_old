...
export class player_commands{
    ...
    movements(){
        ...
        if(this._cursors.up.isUp && this._cursors.down.isUp && this._cursors.right.isUp && this._cursors.left.isUp){
            ...
            else if (this._player.game.input.activePointer.isDown)
            {
                this._player.game.physics.arcade.moveToPointer(this._player, this._player.maxSpeed);

                //  if it's overlapping the mouse, don't move any more
                if (Phaser.Rectangle.contains(this._player.body, this._player.game.input.x, this._player.game.input.y))
                {
